...
from jvcore.layers import DropPath, to_2tuple, trunc_normal_, HardSwish, AdaptiveAvgPool1d, adaptive_avg_pool2d
...
default_cfgs = {
    ...
    'p2t_base': _cfg(url='https://github.com/yuhuan-wu/P2T/releases/download/v1.0/p2t_base.pth'),
    'p2t_large': _cfg(url='https://github.com/yuhuan-wu/P2T/releases/download/v1.0/p2t_large.pth'),
}


class IRB(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, ksize=3, act_layer=HardSwish, drop=0.):
        super().__init__()
...
class PoolingAttention(nn.Module):
    ...
    def execute(self, x, H, W, d_convs=None):
        B, N, C = x.shape
        ...
        for (pool_ratio, l) in zip(self.pool_ratios, d_convs):
            pool = adaptive_avg_pool2d(x_, (round(H/pool_ratio), round(W/pool_ratio)))
            pool = pool + l(pool) # fix backward bug in higher torch versions when training
            pools.append(pool.view(B, C, -1))
        
        pools = jt.contrib.concat(pools, dim=2)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mlp = IRB(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=HardSwish, drop=drop, ksize=3)
...
class PyramidPoolingTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 320, 512],
        ...
        # transformer encoder
        dpr = [x.item() for x in jt.misc.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
        cur = 0
...
def _create_p2t(variant, pretrained, **kwargs):
    return build_model_with_cfg(
        PyramidPoolingTransformer, variant, default_cfg=default_cfgs[variant], pretrained=pretrained, **kwargs)
...
def p2t_tiny(pretrained=False, **kwargs):
    model_args = dict(
        ...
        **kwargs)
    return _create_p2t('p2t_tiny', pretrained, **model_args)
...
def p2t_small(pretrained=True, **kwargs):
    model_args = dict(
        ...
        **kwargs)
    return _create_p2t('p2t_small', pretrained, **model_args)
...
def p2t_base(pretrained=False, **kwargs):
    model_args = dict(
        ...
        **kwargs)
    return _create_p2t('p2t_base', pretrained, **model_args)
...
def p2t_large(pretrained=False, **kwargs):
    model_args = dict(
        ...
        **kwargs)
    return _create_p2t('p2t_large', pretrained, **model_args)
