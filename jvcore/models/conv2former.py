...
default_cfgs = {
    'conv2former_n': _cfg(url=''),
    'conv2former_t': _cfg(url=''),
    'conv2former_s': _cfg(url=''),
    'conv2former_b': _cfg(url=''),
    'conv2former_l': _cfg(url=''),
}
...
def _create_c2f(variant, pretrained, **kwargs):
    return build_model_with_cfg(
        Conv2Former, variant, default_cfg=default_cfgs[variant], pretrained=pretrained, **kwargs
    )

@register_model
def conv2former_n(pretrained=False, **kwargs):
    model_args = dict(
        kernel_size=11, dims=[64, 128, 256, 512], mlp_ratios=[4, 4, 4, 4], depths=[2, 2, 8, 2], **kwargs
    )
    return _create_c2f('conv2former_n', pretrained, **model_args)

@register_model
def conv2former_t(pretrained=False, **kwargs):
    model_args = dict(kernel_size=11, dims=[72, 144, 288, 576], mlp_ratios=[4, 4, 4, 4], depths=[3, 3, 12, 3], **kwargs)
    return _create_c2f('conv2former_t', pretrained, **model_args)

@register_model
def conv2former_s(pretrained=False, **kwargs):
    model_args = dict(kernel_size=11, dims=[72, 144, 288, 576], mlp_ratios=[4, 4, 4, 4], depths=[4, 4, 32, 4], **kwargs)
    return _create_c2f('conv2former_s', pretrained, **model_args)

@register_model
def conv2former_b(pretrained=False, **kwargs):
    model_args = dict(kernel_size=11, dims=[96, 192, 384, 768], mlp_ratios=[4, 4, 4, 4], depths=[4, 4, 34, 4], **kwargs)
    return _create_c2f('conv2former_b', pretrained, **model_args)

@register_model
def conv2former_l(pretrained=False, **kwargs):
    model_args = dict(kernel_size=11, dims=[128, 256, 512, 1024], mlp_ratios=[4, 4, 4, 4], depths=[4, 4, 48, 4], **kwargs)
    return _create_c2f('conv2former_l', pretrained, **model_args)
